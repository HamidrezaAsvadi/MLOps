...
class InferenceConfig:
    # -------- Output controls --------
    QUIET: bool = False               # True => suppress all debug/info/summary prints
    PRINT_IDLE_HEADER: bool = True  # True => prints "[device] IDLE OUTPUT:" header
    PRINT_NO_IDLE: bool = True      # True => prints "(no idle detected)" when idle_count==0
    DEBUG_EXCEPTIONS: bool = True   # True => print exception text even if QUIET
...
def build_device_features(
    ...
    # (Bugfix: keep tuples consistent: (measure_name, output_column_name))
    for out_measure, col_name in [("Output1", "Output1"), ("HeatRelay", "HeatRelay")]:
